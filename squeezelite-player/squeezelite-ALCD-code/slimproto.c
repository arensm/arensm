...
static bool player_name_fixed = false;
...
#if INTERACTIVE
	pkt.deviceid = interactive.enabled ? 8 : 12; // squeezeslave for text-display mode, otherwise squeezeplay
#else
...
#if INTERACTIVE
static bool cli_is_unreserved(unsigned char value) {
	return (value >= 'a' && value <= 'z') || (value >= 'A' && value <= 'Z') ||
		(value >= '0' && value <= '9') || value == '-' || value == '_' ||
		value == '.' || value == '~';
}

static bool cli_encode(char *target, size_t target_size, const char *source) {
	static const char hex[] = "0123456789ABCDEF";
	size_t used = 0;

	while (*source) {
		unsigned char value = (unsigned char)*source++;
		size_t needed = cli_is_unreserved(value) ? 1 : 3;
		if (used + needed >= target_size) return false;
		if (needed == 1) {
			target[used++] = (char)value;
		} else {
			target[used++] = '%';
			target[used++] = hex[value >> 4];
			target[used++] = hex[value & 0x0f];
		}
	}
	target[used] = '\0';
	return true;
...
/* Device ID 8 selects LMS's text-display SqueezeSlave class. That class does
 * not consume client-originated SETD player-name frames, so enforce an
 * explicit -n name through the LMS CLI after the SlimProto connection exists. */
static void setLMSPlayerName(struct sockaddr_in server, const u8_t mac[6], const char *name) {
	char player_id[18];
	char encoded_id[sizeof(player_id) * 3];
	char encoded_name[PLAYER_NAME_LEN * 3 + 1];
	char command[sizeof(encoded_id) + sizeof(encoded_name) + 8];
	struct pollfd pollinfo;
	sockfd cli_sock;
	size_t length;
	ssize_t written;

	snprintf(player_id, sizeof(player_id), "%02x:%02x:%02x:%02x:%02x:%02x",
		mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
	if (!cli_encode(encoded_id, sizeof(encoded_id), player_id) ||
		!cli_encode(encoded_name, sizeof(encoded_name), name)) {
		LOG_WARN("player name is too long for LMS CLI");
		return;
	}

	server.sin_port = htons(LMS_CLI_PORT);
	cli_sock = socket(AF_INET, SOCK_STREAM, 0);
	if (cli_sock < 0) {
		LOG_WARN("cannot create LMS CLI socket: %s", strerror(last_error()));
		return;
	}
	set_nonblock(cli_sock);
	set_nosigpipe(cli_sock);
	if (connect_timeout(cli_sock, (struct sockaddr *)&server, sizeof(server), 2) != 0) {
		LOG_WARN("cannot set player name through LMS CLI at %s:%d",
			inet_ntoa(server.sin_addr), LMS_CLI_PORT);
		closesocket(cli_sock);
		return;
	}

	length = (size_t)snprintf(command, sizeof(command), "%s name %s\n", encoded_id, encoded_name);
	pollinfo.fd = cli_sock;
	pollinfo.events = POLLOUT;
	if (poll(&pollinfo, 1, 2000) != 1) {
		LOG_WARN("timeout setting player name through LMS CLI");
		closesocket(cli_sock);
		return;
	}
	written = send(cli_sock, command, length, MSG_NOSIGNAL);
	if (written != (ssize_t)length) {
		LOG_WARN("failed setting player name through LMS CLI: %s", strerror(last_error()));
	} else {
		LOG_INFO("set LMS player name: %s", name);
	}
	closesocket(cli_sock);
}
#endif
...
#if INTERACTIVE
static void sendSETDLine(void) {
	struct SETD_linelen pkt;

	memset(&pkt, 0, sizeof(pkt));
	memcpy(&pkt.opcode, "SETD", 4);
	pkt.length = htonl(sizeof(pkt) - 8);
	pkt.id = 0xfe;
	pkt.linelen = (u8_t)interactive.linelen;

	LOG_DEBUG("set LCD line length: %d", interactive.linelen);
	send_packet((u8_t *)&pkt, sizeof(pkt));
}
#endif
...
static void process_setd(u8_t *pkt, int len) {
	...
	if (setd->id == 0) {
		if (len == 5) {
			...
			if (player_name_fixed) {
				// A command-line name is authoritative. Tell the server again
				// instead of replacing it with the server's stored player name.
				sendSETDName(player_name);
				return;
			}
...
static void process_vfdc(u8_t *pkt, int len) {
	interactive_receive_display(&interactive, pkt, (size_t)len);
}
#endif
...
static void slimproto_run() {
	...
	int timeout_limit = 35;

#if INTERACTIVE
	if (interactive.terminal_enabled) {
		wait_timeout = 100;
		timeout_limit = 350;
	}
#endif
	...
	while (running && !new_server) {
		...
		if ((ev = wait_readwake(ehandles, wait_timeout)) != EVENT_TIMEOUT) {
		...
		} else if (++timeouts > timeout_limit) {
...
#if INTERACTIVE
			if (interactive.terminal_enabled) {
				u32_t terminal_code = interactive_read_key(&interactive);
				if (terminal_code == INTERACTIVE_KEY_QUIT) {
					slimproto_stop();
				} else if (terminal_code) {
					_sendIR = true;
					ir_code = terminal_code;
					ir_ts = now;
				}
			}
#endif
...
#if INTERACTIVE
			if (player_name_fixed && player_name[0]) {
				sendSETDName(player_name);
				if (interactive.enabled) {
					usleep(250000);
					setLMSPlayerName(serv_addr, mac, player_name);
				}
			}
#endif
#if INTERACTIVE
			if (interactive.enabled) sendSETDLine();
#endif
