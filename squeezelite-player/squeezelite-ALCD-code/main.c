...
#endif
#if INTERACTIVE
		   "  -T \t\t\tEnable two-line terminal text display\n"
		   "  -y \t\t\tEnable LCDd (lcdproc) text display\n"
		   "  -x \t\t\tEnable compatibility mode for LCDd older than 0.5.4\n"
		   "  -E <host>\t\tConnect to LCDd on host instead of 127.0.0.1\n"
		   "  -w <chars>\t\tSet LCD width (11-99); LCDd detection overrides it when available\n"
#endif
...
#define MAXCMDLINE 512
	...
	while (optind < argc && strlen(argv[optind]) >= 2 && argv[optind][0] == '-') {
		...
		case 'T':
			...
			interactive.terminal_enabled = true;
			break;
		case 'y':
			interactive.enabled = true;
			interactive.lcd_enabled = true;
			break;
		case 'x':
			interactive.enabled = true;
			interactive.lcd_enabled = true;
			...
			break;
		case 'E':
			if (!interactive_set_host(&interactive, optarg)) {
				fprintf(stderr, "Invalid LCDd host: %s\n", optarg);
				exit(1);
			}
			break;
		case 'w':
			if (!interactive_set_width(&interactive, optarg)) {
				fprintf(stderr, "LCD width must be between %d and %d\n", INTERACTIVE_MIN_WIDTH, INTERACTIVE_MAX_WIDTH);
				exit(1);
			}
			break;
#endif
