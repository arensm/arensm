...
void Display::_buildPager(){
  ...
  // Diese Hardwareausfuehrung besitzt das Peak-Meter fest im Displaylayout.
  // Die Laufzeitvorgabe ist absichtlich unabhaengig von einem alten NVS-Wert.
...
void Display::selectPlaylistAtY(uint16_t y) {
  if (!_plwidget || config.playlistLength() == 0) return;
  const int16_t offset = _plwidget->itemOffsetAt(y);
  int selected = (int)currentPlItem + offset;
  const int count = config.playlistLength();
  while (selected < 1) selected += count;
  while (selected > count) selected -= count;
  Serial.printf("##TOUCH.PLAYLIST#: y=%u current=%u offset=%d selected=%d count=%d\n",
                y, currentPlItem, offset, selected, count);
  currentPlItem = selected;
