...
namespace {
constexpr const char* WIFI_PREFS_NAMESPACE = "yoradio-wifi";
constexpr const char* WIFI_PREFS_SSID = "ssid";
constexpr const char* WIFI_PREFS_PASSWORD = "password";

bool loadWifiFromNvs(char* ssid, size_t ssidSize, char* password, size_t passwordSize) {
  Preferences preferences;
  if (!preferences.begin(WIFI_PREFS_NAMESPACE, true)) return false;
  String storedSsid = preferences.getString(WIFI_PREFS_SSID, "");
  String storedPassword = preferences.getString(WIFI_PREFS_PASSWORD, "");
  preferences.end();
  if (storedSsid.isEmpty() || storedSsid.length() >= ssidSize || storedPassword.length() >= passwordSize) return false;
  strlcpy(ssid, storedSsid.c_str(), ssidSize);
  strlcpy(password, storedPassword.c_str(), passwordSize);
  return true;
}

bool saveWifiToNvs(const char* ssid, const char* password) {
  Preferences preferences;
  if (!preferences.begin(WIFI_PREFS_NAMESPACE, false)) return false;
  bool saved = preferences.putString(WIFI_PREFS_SSID, ssid) > 0;
  // Ein leeres Passwort ist bei einem offenen WLAN gueltig.
  preferences.putString(WIFI_PREFS_PASSWORD, password);
  preferences.end();
  return saved;
}
}
...
bool Config::saveWifiFromNextion(const char* post){
  char ssid[30] = {};
  char password[40] = {};
  if (!parseSsid(post, ssid, password)) return false;
  if (!saveWifiToNvs(ssid, password)) return false;
  delay(100);
  ESP.restart();
  return true;
}
...
bool Config::initNetwork() {
  char nvsSsid[30] = {};
  char nvsPassword[40] = {};
  if (loadWifiFromNvs(nvsSsid, sizeof(nvsSsid), nvsPassword, sizeof(nvsPassword))) {
    strlcpy(ssids[0].ssid, nvsSsid, sizeof(ssids[0].ssid));
    strlcpy(ssids[0].password, nvsPassword, sizeof(ssids[0].password));
    ssidsCount = 1;
    return true;
  }
  ...
  file.close();
  // Bestehende Installationen einmalig aus wifi.csv nach NVS migrieren.
  if (ssidsCount > 0) saveWifiToNvs(ssids[0].ssid, ssids[0].password);
  return true;
}
