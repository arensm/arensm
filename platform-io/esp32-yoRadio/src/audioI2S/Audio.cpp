...
uint16_t Audio::get_VUlevel(uint16_t dimension){
  ...
  long mappedL = map(vuLeft, config.vuThreshold, 0, 0, dimension);
  long mappedR = map(vuRight, config.vuThreshold, 0, 0, dimension);
  if(mappedL < 0) mappedL = 0;
  if(mappedR < 0) mappedR = 0;
  if(mappedL > dimension) mappedL = dimension;
  if(mappedR > dimension) mappedR = dimension;
  uint8_t L = mappedL;
  uint8_t R = mappedR;
  return (L << 8) | R;
}
