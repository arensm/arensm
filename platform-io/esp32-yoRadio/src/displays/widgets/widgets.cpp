...
void VuWidget::init(WidgetConfig wconf, VUBandsConfig bands, uint16_t vumaxcolor, uint16_t vumincolor, uint16_t bgcolor) {
  ...
  _bands = bands;
#ifdef CYD_LEFT_PEAK_METER
  // Auf dem CYD ohne PSRAM direkt ins Display zeichnen. Ein RGB565-Canvas
  // fragmentiert den knappen Heap und kann HTTPS/SSL-Audiostreams stoppen.
  _canvas = nullptr;
#else
...
#ifdef CYD_LEFT_PEAK_METER
  // Speicherfreie Direktzeichnung der beiden vertikalen Balken.
  // Der Pegelwert bezeichnet den von oben auszublendenden Bereich.
  static uint32_t lastPeakLog = 0;
  if (millis() - lastPeakLog >= 5000) {
    lastPeakLog = millis();
    Serial.printf("##PEAK#: playing=%d threshold=%u left=%u right=%u\n",
                  played, config.vuThreshold, dimension - L, dimension - R);
  }
  for(int i=0; i<dimension; i += dimension/_bands.perheight){
    const uint8_t band = i / (dimension / _bands.perheight);
    bandColor = band < 2 ? 0xF800 : (band < 5 ? 0xFFE0 : 0x07E0);
    const uint8_t columnWidth = 4;
    const uint8_t innerGap = 1;
    const uint16_t leftColor = i >= measL ? bandColor : _bgcolor;
    const uint16_t rightColor = i >= measR ? bandColor : _bgcolor;
    const uint16_t leftX = _config.left;
    const uint16_t rightX = _config.left + _bands.width + _bands.space;
    // Zwei schmale Saeulen je Kanal: L L | R R.
    dsp.fillRect(leftX, _config.top + i, columnWidth, h, leftColor);
    dsp.fillRect(leftX + columnWidth + innerGap, _config.top + i,
                 columnWidth, h, leftColor);
    dsp.fillRect(rightX, _config.top + i, columnWidth, h, rightColor);
    dsp.fillRect(rightX + columnWidth + innerGap, _config.top + i,
                 columnWidth, h, rightColor);
  }
  return;
#endif
...
#ifdef CYD_LEFT_PEAK_METER
        // Oben rot, in der Mitte gelb und unten gruen wie beim klassischen
        // Stereo-Peak-Meter. i=0 bezeichnet bei diesem Widget die Oberkante.
        const uint8_t band = i / (dimension / _bands.perheight);
        bandColor = band < 2 ? 0xF800 : (band < 5 ? 0xFFE0 : 0x07E0);
#else
...
int16_t PlayListWidget::itemOffsetAt(uint16_t y) {
  int16_t row = 0;
  if ((int)y > _plYStart) row = ((int)y - _plYStart) / _plItemHeight;
  if (row >= _plTtemsCount) row = _plTtemsCount - 1;
  return row - _plCurrentPos;
}
